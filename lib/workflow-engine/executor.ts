...
const nodeExecutors: Record<string, NodeExecutor> = {
  ...
  switch: async (node, input) => {
    ...
    const cases = node.data.parameters.cases as {
      value: string;
      output: string;
    }[];
  ...
  "ai-chat": async (node, input) => {
    const { model, systemPrompt, prompt, temperature } = node.data
      .parameters as {
      model: string;
      systemPrompt?: string;
      prompt: string;
  ...
  async execute(): Promise<WorkflowExecution> {
    ...
    try {
      ...
      this.execution.duration =
        this.execution.finishedAt.getTime() -
    ...
    try {
      ...
      for (const edge of outgoingEdges) {
        ...
        if (node.definitionId === "if-condition") {
          ...
          if (
            (result && edge.sourceHandle === "true") ||
            (!result && edge.sourceHandle === "false")
          ) {
        ...
        // Handle switch routing
          ...
          const matchedOutput = (output as { matchedOutput: string })
            .matchedOutput;
